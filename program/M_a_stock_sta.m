...
if strcmp(run_mod,'m')
    key_str = 'S42 A股多 寻优参数';
else
    key_str = 'S42 A股多 固定参数';
end

sql_str = 'select distinct(symbol) from S42.S42_a_stock where c_m = "%s"';
symbols = fetchmysql(sprintf(sql_str,run_mod),2);
T_symbols = length(symbols);
sql_str1 = 'select tradeDate,chg from S42.S42_a_stock where symbol = "%s" and c_m = "%s"';
...
%     %补充手续费计算，本来应该1.5/1000，我用了5/100000
%     ind = ~eq(r_day,0);
%     r_day(ind) = r_day(ind) +5/100000-1.5/1000;
