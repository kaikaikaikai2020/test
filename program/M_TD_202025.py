...
数据修正
 ...
 delete from yuqerdata.yq_MktEqudAdjAfGet where tradeDate<="2020-08-21";
 insert into yuqerdata.yq_MktEqudAdjAfGet select * from  yuqerdata.MktEqudAdjAfGetF1S53 where isOpen=1;
@author: Asus
@update adair
"""
...
db_pro = 'data_pro'
#eng_str='mysql+pymysql://%s:%s@localhost:%d/%s?charset=utf8' % (user_name,pass_wd,port,db_name)
eng_str='mysql+pymysql://%s:%s@localhost:%d/%s?charset=utf8' % (user_name,pass_wd,port,db_pro)
eg_pro = create_engine(eng_str)
...
def get_a_stock_tradeDate_update(index,begin='2000-01-01',end='2033-01-01'):
    #每日数据
    sql_str_index = '''select ticker as symbol,tradeDate,openPrice as openIndex,highestPrice as highestIndex,
    lowestPrice as lowestIndex,closePrice  as closeIndex from yq_MktEqudAdjAfGet
    where ticker = "%s"  and tradeDate>="%s" and 
    tradeDate<="%s" order by tradeDate'''
    sql_str_index = sql_str_index % (index,begin,end)
    hs300_index = pd.read_sql(sql_str_index,engine)
    hs300_index = hs300_index.sort_values('tradeDate')
...
def a_stock_test_S42(index,run_mod='m',method_sel ='a_stock_test_S42'):
    ...
    print ("%s begin ......"%index)
    hs300_index = get_a_stock_tradeDate_update(index,begin,end)
    m,_ = hs300_index.shape
    print(m)
    if m > 245*10:
        begin = hs300_index.tradeDate.min()
...
def get_index_comS42(ticker,index_2,begin='2000-01-01',end='2033-01-01'):
    #每日数据
    sql_str_index = '''select ticker as symbol,tradeDate,openPrice as openIndex,highPrice as highestIndex,
    lowPrice as lowestIndex,closePrice  as closeIndex from main_index_s42
    where index_id = "%s" and ticker ="%s" and tradeDate>="%s" and 
    tradeDate<="%s" order by tradeDate'''
    sql_str_index = sql_str_index % (index_2,ticker,begin,end)
    hs300_index = pd.read_sql(sql_str_index,eg_pro)
    hs300_index = hs300_index.sort_values('tradeDate')
    return hs300_index   
#美股按照老框架运行
def classic_TDmodel(input_data,ls_mod='ls'):
    index,index_2,cost,run_mod=input_data
    print('%s-%s' % (index,index_2))
    #print(method_sel)
    ...
    if ls_mod=='ls':
        ...
        tn = 's42_index_test_lo'
        strategy_type='lo'
    
    print ("%s begin ......"%index)
    hs300_index = get_index_comS42(index,index_2,begin,end)
    m,_ = hs300_index.shape
    print(m)
    if m > 245*5:
        begin = hs300_index.tradeDate.min()
        ...
        sub_sql_str = 'select tradeDate from %s where symbol = "%s" and c_m = "%s" and index_id="%s" order by tradeDate desc limit 1'
        t0 = pd.read_sql(sub_sql_str % (tn,index,run_mod,index_2),engine42)
        if len(t0)>0:
            t0=t0.tradeDate[0]
        else:
            t0=date(2000,1,1)
        ...
        return []        
#美股按照新框架运行暂时未修改
def new_TD(index,index_2,cost = 0.5/10000,run_mod='f'):
    #print(method_sel)
    tn ='s42_index_test_new'
...
if __name__ == '__main__':    
    tn0='main_index_s42'    
    index=['AS51', 'hscei', 'hsi', 'msci', 'nky', 'topix','Nifty']
    fee = dict(zip(index,[1/10000*2,1.2/1000*2,1.2/1000*2,3/10000*2,1/10000*2,1/10000*2,1/1000*2]))
    sql_str1 = 'select distinct(ticker) as t from %s where index_id ="%s"'
    for sub_index in index:
        ticker = pd.read_sql(sql_str1 % (tn0,sub_index),eg_pro)
        ticker = ticker.t.tolist()
        
        sub_index_pool = [sub_index] * len(ticker)
        sub_fee_pool = [fee[sub_index]] * len(ticker)
        sub_mod_pool = ['f'] * len(ticker)
        
        sub_mod_pool2 = ['lo'] * len(ticker)
        
        pool = multiprocessing.Pool(num_core) 
        pool.map(classic_TDmodel, zip(ticker,sub_index_pool,sub_fee_pool,
                                        sub_mod_pool,sub_mod_pool2))
        pool.close()
        pool.join()
        
        sub_mod_pool2 = ['ls'] * len(ticker)        
        pool = multiprocessing.Pool(num_core) 
        pool.map(classic_TDmodel, zip(ticker,sub_index_pool,sub_fee_pool,
                                        sub_mod_pool,sub_mod_pool2))
        pool.close()
        pool.join()
        #sub_ticker=ticker[0]
        #classic_TDmodel(sub_ticker,sub_index,fee[sub_index])
