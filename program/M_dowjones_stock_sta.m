...
if strcmp(run_mod,'m')
    key_str = 'S42 dowjones 寻优参数多空';
else
    key_str = 'S42 dowjones 固定参数多空';
end

sql_str = 'select distinct(symbol) from S42.S42_dowjones where c_m = "%s"';
symbols = fetchmysql(sprintf(sql_str,run_mod),2);

T_symbols = length(symbols);
sql_str1 = 'select tradeDate,chg from S42.S42_dowjones where symbol = "%s" and c_m = "%s" and g_num = 1';
