...
sql_str = 'select distinct(symbol) from S42.S42_american_stock where c_m = "%s"';
symbols = fetchmysql(sprintf(sql_str,run_mod),2);
T_symbols = length(symbols);
sql_str1 = 'select tradeDate,chg from S42.S42_american_stock where symbol = "%s" and c_m = "%s"';
