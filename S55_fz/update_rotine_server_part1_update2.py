...
if __name__ == '__main__':
    ...
    #tref = [i for i in tref if i >'2019-01-01']
    #tref = tref[0:100]
    ...
    #for i in zip(t0_1,tt_1):
    #    pool.apply_async(get_databy_day,[i])
    #pool.close()
    #pool.join()
    pool = ThreadPool(processes=num_core)
    temp = pool.map(get_databy_day, zip(t0_1,tt_1))
    pool.close()
    pool.join()     
    '''
    pool = multiprocessing.Pool(num_core)   
    ...
    for i,sub_date in enumerate(month_date):
        obj_clock.use('begin  %s-%s' %(i,sub_date))
        frame_list.append(pool.apply_async(SwIndu_update,
                                                [indu,sub_date]))            
    obj_clock.use('complete  %s-%s' %(i,sub_date))    
    pool.close()
    pool.join()
    '''