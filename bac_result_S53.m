classdef bac_result_S53 < handle
    ...
    methods(Static)
        function update_signal_p1()
            sprintf('开始更新S53P1(MHKQ因子择时模型在A股中的运用)因子数据，月末更新（更新时耗时10分钟左右）')
            dos('python S53_bac_tool1.py')
            sprintf('完成更新S53P1(MHKQ因子择时模型在A股中的运用)因子数据')
        end
        function update_signal_p2()
            sprintf('开始更新S53P2(细分行业下的多因子选股模型)因子数据，月末更新（更新时耗时10分钟左右）')
            dos('python S53_bac_tool2_update2.py')
            sprintf('完成更新S53P2(细分行业下的多因子选股模型)因子数据')
        end        
        function bac_S53P1()
