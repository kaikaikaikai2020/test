...
function re = get_sub_dmd_result(t0,tref,tref_300,close_price_300_return_f,close_price_300_return_b,ticker_300,t_300,datenum_300,num3)
    ...
    t3 = get_trading_date_interval(tref,t0,num3*2); %过去N天数据
    [X,~] = get_history_300_data(ticker,t3(1),t3(end),length(t3));
    y = close_price_300_return_f(max(1,ind+num3*2):ind);
    if length(y)<abs(num3*2)+1
        temp = abs(num3*2)+1-length(y);
        y = [zeros(temp,1);y];
    end
    wids = 50:2:100;
    ...
    for j = 1:length(wids)
        ...
        xy_data = zeros(100,2);
        for k = 1:100        
            sub_X = X(:,end-k-wid+1:end-k+1);
            [mu,~,~,R2] = dmd_method(sub_X);
            xy_data(k,:) = [abs(mu(1,1)),y(end-k+1)];
            if eq(k,1)
