%S46-part1 五等分，周度
classdef bac_result_S49<handle
    properties
        obj_S37 = bac_tool_S37();
        symbol_pool = 'symbol_pool_S49';
        factors = {'S49_f1'};
        factors_full = {'S49_f1'};
        %tns = {'S46.S46f1'};
        tns_info = {'S49行业轮动因子'};
        zn_sel = 1;
        f_dir = {' '};
        group_return_tn = {'factor_group5_return_S49'};
    end
    methods
        function get_all_results(obj)
            %part1
            %研报数据更新
            dos('python S49_bac_tool.py')
            %回测
            bac_obj = bac_tool_ac_p();
            bac_obj.bac_S49_p1();
            %生成表格 股票池 收益 
            obj.create_table()
            %选股和写入收益
            obj.select_symbol_f2()
            %选股 需要补充
            obj.get_group_return()
            %生成报告
            obj.get_report()            
        end
        function select_symbol_f2(obj)
            key_str = 'S49 根据行业排名获取股票池';
            f_name = 'S49_f1';
            dN = 'S37';
            tn = 'symbol_pool_s49';
            var_info = {'factor_name','tradingdate','symbol','id_a','id500','id300'};
            tN = sprintf('%s.%s',dN,tn);
            tN_from = 'S49.group_p2_py';
            index_pool = {'a','000905','000300'};
            %现有结果时间
            sql_str = 'select distinct(tradingdate) from %s';
            tref_complete = fetchmysql(sprintf(sql_str,tN),2);

            tref = 'select distinct(tradeDate) from %s';
            tref = fetchmysql(sprintf(tref,tN_from),2);
            ...
            T = length(tref);
            sql_str_indus_re = 'select ticker,group1+1 from %s where tradeDate = "%s" and group1 is not null order by group1';
            parfor i = 1:T
                sub_t = tref{i};
                sub_indus = fetchmysql(sprintf(sql_str_indus_re,tN_from,sub_t),2);
                sub_symbol = obj.get_industry_class_update(sub_t);
                y = zeros(length(sub_symbol(:,1)),3);
                for j = 1:length(index_pool)
                    if eq(j,1)
                        for k = 1:size(sub_indus,1)
                            ia = strcmp(sub_symbol(:,2),sub_indus(k,1));
                            y(ia,j) = cell2mat(sub_indus(k,2));
                        end
                    else
                        sub_sub_indexpool = yq_methods.get_index_pool(index_pool{j},sub_t);
                        [~,ia] = intersect(sub_symbol(:,1),sub_sub_indexpool);
                        sub_sub_symbol = sub_symbol(ia,:);
                        sub_y = zeros(size(sub_sub_symbol(:,1)));
                        for k = 1:size(sub_indus,1)
                            ia1 = strcmp(sub_sub_symbol(:,2),sub_indus(k,1));
                            sub_y(ia1) = cell2mat(sub_indus(k,2));
                        end
                        y(ia,j) = sub_y;            
                    end

                end
                sub_f = [sub_symbol(:,[1,1,1]),num2cell(y)];
                del_ind = eq(y(:,1),0);
                sub_f(del_ind,:) = [];
                y(del_ind,:) = [];
                [~,ia] = sort(y(:,1));
                ...
                sub_f(:,2) = tref(i);
                if ~isempty(sub_f)
                    datainsert_adair(tN,var_info,sub_f)
                end
        ...
        function get_group_return(obj)            
            ...
            for i = 1:T_f
                ...
                parfor j = 1:T_tref
                    ...
                    chg_re{j} = f';                    
                    sprintf('S49 每日回测结果写入数据库-%s %d-%d %d-%d',sub_t,j,T_tref,i,T_f)
                end
                f = [chg_re{:}]';
                if ~isempty(f)
        ...
        function get_report(obj)           
            ...
            file_name = sprintf('%s行业轮动表现%s',key_str,datestr(now,'yyyy-mm-dd'));
            pn0 = fullfile(pwd,'计算结果');
            if ~exist(pn0,'dir')
                mkdir(pn0);
            end
    ...
    methods(Static)      
        function x = get_industry_class_update(t)
            str_str1 = ['select ticker,industryName1 from yuqerdata.yq_industry where ',...
