...
def div_list(ls1,n):
	if not isinstance(ls1,list) or not isinstance(n,int):
		return []
	ls_len = len(ls1)
	if n<=0 or 0==ls_len:
		return []
	if n > ls_len:
		...
		for i in range(0,(n-1)*j,j):
			ls_return.append(ls1[i:i+j])
		#算上末尾的j+k
		ls_return.append(ls1[(n-1)*j:])
		return ls_return
...
def get_tradingdate_adair(tt):
    trefHS=DataAPI.TradeCalGet(exchangeCD=u"XSHG,XSHE",beginDate=u"",endDate=tt,isOpen=1,field=u"",pandas="1")
    trefHS_M=trefHS[trefHS.isMonthEnd==1].calendarDate.unique().tolist()
    trefHS_M = [i.replace('-','') for i in trefHS_M]
    trefHS_d = trefHS.calendarDate.unique().tolist()
    trefHS_d = [i.replace('-','') for i in trefHS_d]
    return trefHS_d,trefHS_M
...
class uq_methods:
    ...
    def get_rontie_1d_us(self,t0,tt,para_sel=True):
        ...
        #1 sub_x=DataAPI.MktUsequdGet(ticker=u"",tradeDate=sub_t,beginDate=u"",endDate=u"",exchangeCD=u"",field=u"",pandas="1")
    ...
    def get_rontie_1d(self,t0,tt,para_sel=True):        
        ...
        def get_MktStockFactorsOneDayGet_full():
            ...
            if fn2_d1 not in list_files():
                ...
                return None
        #6 指数成分股   
        def  IdxConsGet_update():
            fn_d2 = 'IdxConsGetS57_%s' % tt
            if '%s.csv' % fn_d2 in list_files():
                return
            X =[]
            if tt in trefHS_M:
                tickers_A = DataAPI.IdxConsGet(secID=u"",ticker=["000002","000300","000905","000001","000985"],isNew=u"",intoDate= tt,
                                     field=u"",pandas="1")
                tickers_A['tradeDate'] = tt
                X.append(tickers_A)
                if len(X)>0:
                    X = pd.concat(X)
    ...
    def get_rontie_0(self,t0,tt,para_sel=True):
        ...
        def get_TradeCalGet():
            ...
            if fn2_d2 in list_files():
                return
            x0=DataAPI.TradeCalGet(exchangeCD=u"XSHG,XSHE",beginDate=u"",endDate=u"",isOpen=u"",field=u"",pandas="1")
            x = x0[x0.calendarDate<=datetime.datetime.strptime(tt,'%Y%m%d').strftime('%Y-%m-%d')]
            save_data_adair(fn_d2,x)            
            
            x0=x0[['calendarDate','isOpen']]
            x0=x0[x0.isOpen==1]
            temp=x0.calendarDate.unique()
            x = pd.DataFrame({'tradingdate':temp})
            fn_d2 = 'tradingdate_%s' % tt
            save_data_adair(fn_d2,x)
    ...
    def get_rontie_halfyear(self,t0,tt,para_sel=True):
        ...
        def get_FundHoldingsGet():
            ...
            if '%s.csv' % fn_d2 in list_files():
                return
            v = div_list(fund_secID,20)
            #分为20段，然后来执行
            x=[]
            for sub_secID in v:
                sub_x=DataAPI.FundHoldingsGet(secID=sub_secID,ticker=u"",reportDate=u"",
                                              beginDate=t0,endDate=tt,secType="",
        ...
        if para_sel:
            ...
            for i in ind_p:
                do_update(i)
    ###3month
    def get_rontie_3mon(self,t0,tt,para_sel=True):
...
if __name__ == '__main__':
    ...
    t_hy0 = datetime.datetime.strftime( datetime.datetime.today()-datetime.timedelta(60),'%Y%m%d')
    t_hy1 = datetime.datetime.strftime( datetime.datetime.today()-datetime.timedelta(180),'%Y%m%d')
    ...
    trefHS_M = [i.replace('-','') for i in trefHS_M if i>=t0.replace('-','')]
    ...
    while tryind <=10 and not OK:
        ...
        try:
            ...
            uq_m.get_rontie_3mon(t_hy0,tt)
            uq_m.get_rontie_halfyear(t_hy1,tt)
