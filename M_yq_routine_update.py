...
for sub_fn in fns_csv:
    ...
    elif 'FundHoldingsGet_S51' in sub_fn:     ###########缺口，后续可以替换掉
...
if len(fns_data[data_id])>0:
    ...
    if not x2.empty:
        #x2.rename(columns={'ticker':'symbol'},inplace=True)
        x2.drop_duplicates(subset=['ticker','tradeDate','STflg'], keep='first', inplace=True)
        x2.to_sql(tn1,engine,if_exists='append',index=False,chunksize=3000)
...
if len(fns_data[sub_id])>0:
    for sub_fn in fns_data[sub_id]:
        ...
        t = pd.read_sql(sql_str1,engine)
        if not t.empty:
            tt_str = str(t.enddate[0])
        else:
            tt_str = '1990-01-01'
        x2 = x.loc[x.endDate>tt_str]
        if not x2.empty:
            x2.rename(columns={'return':'weekreturn'},inplace=True)
            x2.to_sql(tn1,engine,if_exists='append',index=False,chunksize=3000)
            print('每周股票后复权数据更新至%s' % x2.endDate.max())        
        else:
            print('每周股后复权数据已经是最新的%s，无需更新' % tt_str)
        os.remove(sub_fn)
#指数周行情
sub_id = 29
if len(fns_data[sub_id])>0:
...
if len(fns_data[sub_id])>0:
    ...
    for sub_fn in fns_data[sub_id]:
        ...
        if len(x)>0:
            ...
            print('%s已经更新至%s' % (info,t2))
        os.remove(sub_fn)       
#39季度发布，检索更偏向于财务数据方法，移除
        
#MktCmeFutdGet_S50
#CME期货日行情 
#获取CME Group所有期货合约日行情信息，包含芝加哥商业交易所 (CME)、 芝加哥期货交易所(CBOT)、
#纽约商业交易所 (NYMEX)、纽约商品交易所 (COMEX)。 201611
sub_id = 40
if len(fns_data[sub_id])>0:
    info = 'CME期货日行情'
    for sub_fn in fns_data[sub_id]:
