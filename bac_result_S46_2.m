%有两个因子基于python合成的因子
classdef bac_result_S46_2<handle
    properties
        obj_S37 = bac_tool_S37();
        symbol_pool = 'symbol_pool_S46';
        factors = {'sum_post_gb_15','sum_post_gb_mkt_5'};
        factors_full = {'S46_p2_f1','S46_p2_f2'};
        tns = {'S46.S46_p2_f1','S46.S46_p2_f2'};
        tns_info = {'发帖数量因子1','发帖数量因子2'};
        zn_sel = [1,1];
        f_dir = {'-','-'};
    ...
    methods
        function get_all_results(obj)           
            %part2
            %升级基础因子数据
            dos('python S46_part2_factor_update.py')
            %生成表格 股票池 收益 
            obj.create_table()
            %选股和写入收益
            obj.select_symbols_S46()
        ...
        %按照池子选择
        function select_symbols_S46(obj)            
            T_f = length(obj.tns);
            ...
            for i = 1:T_f
                ...
                parfor j = 1:T_tref_do                    
                    ...
                    sprintf('S46-part2因子%0.2d-%s选股结果写入数据库',i,sub_t)                    
                end                
            end
        end
        %中性化等操作
        ...
        function get_group_return(obj)            
            ...
            for i = 1:T_f
                ...
                parfor j = 1:T_tref
                    ...
                    chg_re{j} = f';                    
                    sprintf('S46-part2-每日回测结果写入数据库-%s %d-%d %d-%d',sub_t,j,T_tref,i,T_f)
                end
                f = [chg_re{:}]';
                if ~isempty(f)
        ...
        function get_report(obj)           
            ...
            for i = 1:T_factor
                ...
                for j = 1:3
                    ...
                    [v0,v_str0] = curve_static(cumprod(1+r_day(:,end)),[],false);
