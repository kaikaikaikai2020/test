...
def get_IdxCons(intoDate,ticker='000300'):
    #nearst 时间
    sql_str1 = '''select symbol from IdxCloseWeightGet where ticker = "%s"
            and tradingdate = (select tradingdate from IdxCloseWeightGet where 
...
def get_calender_range(begin, end):
    sql_str = """select tradeDate from yq_index where symbol = "000001" 
...
#获取所有交易日历
def get_calender():
    sql_str = '''select tradeDate from yq_index where symbol = "000001" order by tradeDate'''
    x=pd.read_sql(sql_str,engine)
    x=x['tradeDate'].values
    #b=[i.strftime('%Y-%m-%d') for i in x]
    return x
...
def get_month_calender(begin = '2000-01-01'):
    sql_str = '''select endDate from yq_index_month where symbol = "000001" and endDate>="%s" order by endDate''' % (begin)
    x=pd.read_sql(sql_str,engine)
    x=x['endDate'].values
    #b=[i.strftime('%Y-%m-%d') for i in x]
    return x
...
def get_week_month_tradeDate(start_date,end_date):
    ...
    # 获取月末交易日
    sql_str_calender = """select * from yuqer_cal where exchangeCD = "%s" 
...
def get_industry_class(t):
    sql_str1 = '''select ticker,industryID1 from yq_industry where 
...
def get_industry_class_update(t,fileds_str = 'ticker,industryID1'):
    sql_str1 = '''select %s from yq_industry where 
                industryVersionCD="010303" and intodate <= "%s" and 
...
#ST info
def SecSTGet(t0,t1):
    sql_tmp = 'select * from st_info where tradeDate>="%s" and tradeDate<="%s"'
