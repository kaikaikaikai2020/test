classdef bac_result_S40<handle
    ...
    methods(Static)
        ...
        function M_Astock_Indicator()
            ...
            symbols = {'sh000016','sh000300'};
            symbols_index = {'50','300'};
...
%             end
        ...
        function M_future_CF()
            ...
            setpixelposition(h,[223,365,1345,600]);
            box off
            title(sprintf('%s-国内期货组合曲线',title_str))
            %每年收益
            t_y = year(t);
            t_y_u = unique(t_y);
            r_year = zeros(size(t_y_u));
            ...
            if write_sel
                ...
                obj_wd = wordcom(fullfile(pn_write,sprintf('%s组合%s.doc',key_str,datestr(now,'yyyymmdd'))));
                xls_fn = fullfile(pn_write,sprintf('%s组合%s.xlsx',key_str,datestr(now,'yyyymmdd')));
